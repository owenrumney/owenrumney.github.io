...
I have had an on/off relationship with Kubernetes for a number of years and want to spend some really getting to know how to use it, run it and secure it.
...
We will be using `kubectl` later to interact with the cluster, so this is also required.
...
Now we have this, we want to check against the checksum file
...
We use the checksums file to validate the binary is as it should be

```bash
echo "$(cat kubectl.sha256)  kubectl" | sha256sum --check
```

Assuming we were told it was `kubectl: OK` we can continue to install.
...
which should give us output similar to 
...
Once I have downloaded, I can installed the command
...
Once it has been installed we can run to check 
...
To do this, we use the `cluster` sub command
...
Now that the cluster is running we can use `kubectl` to get some more information about the cluster
...
We know that we have 3 nodes - so lets get some inforamtion about them too
...
The last step in this bootstrapping post is to check we can run a container - so I'm going to do a basic command to run a new pod called `nginx` using the `nginx` image
...
which hopefully should give us some output letting us know our pod has been created
...
We can check the status of the pod using `kubectl`

```bash
kubectl describe pod nginx
```

Which will give us a verbose output about the pod
...
Start Time:   Fri, 08 Apr 2022 15:54:28 +0100
Labels:       run=nginx
Annotations:  <none>
Status:       Running
IP:           10.42.2.4
...
Containers:
  nginx:
    ...
    Host Port:      <none>
    State:          Running
      Started:      Fri, 08 Apr 2022 15:54:34 +0100
    Ready:          True
    Restart Count:  0
    Environment:    <none>
    Mounts:
...
We can see from this that the pod was deployed on `server-2` and pulled the latest `nginx` image before successfully starting

### Cleaning Up

Now that we know that everything seems to be okay, we can tear it all down before starting to do something more useful in the next blog post.
